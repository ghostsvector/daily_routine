#!/bin/bash
# Build the Flutter Linux app from scratch, package it as a .deb, and
# install it — used by CI and for building from a local source checkout.
# For just installing the current published release, use install_deb.sh
# instead (downloads from GitHub, no local build needed).
set -euo pipefail
